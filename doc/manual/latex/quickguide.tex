...
applications, together with their corresponding parameters, that are
...
command-line (this is why we call it \emph{command-line template}!).
%
...
\texttt{name(...)}. 
%
This script is provided just explain how an application
...
the \ei Output Language (\eiol), which is a text-based language
...
the output to the console area and thus we need a command to
...
\medskip
\noindent
The output of \texttt{myapp.sh} is given in \eiol, because at Line 5 we
...
which the marker should be added (as received from the server).
%
...
\medskip
\noindent
which should be at line 19, and replace it by
...
(i.e., the corresponding HTML) the identifier \texttt{files}, and will
...
files in the console area (anywhere in the yellow region) should open
a dialog box. Execute \texttt{myapp.sh} in a shell first to check that
it works correctly, and then execute the \texttt{MFA} application from
