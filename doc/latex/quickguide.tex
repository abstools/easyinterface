...
The \ei server is written in PHP, and thus requires an HTTP server to
...
\lst{method} declares that this application is to be execute using a
...
a temporal directory, e.g., in \texttt{/tmp}, replaces \lst{_ei_files}
...
click on the \refreshoutline button, you will get a tree-view of with
this program entities (if you use \refreshoutline from the context
...
%
The server will change \lst{_ei_root} by the name of the temporal
