...
applications available in the web-client.
%
...
has also some parameters that can be set to some value.
%
...
parameters (thos in the settings section) and the file that is
currently active in the editor area.
%
...
\url{http://localhost/ei/server}. Let us add a simple ``Hello World''
application.
...
\lst{method} declares that this application should be executed from a
...
our discussion now (see Section~\ref{} for more details). 
%
Note also
...
\medskip
\begin{lstlisting}
> myapp.sh -f file1.abs file2.abs file3.abs -e sum.c:main sum.c:sum
\end{lstlisting}
...
\medskip
\begin{lstlisting}
> myapp.sh -f file1.abs file2.abs file3.abs -e sum.c:main sum.c:sum -s -c words
\end{lstlisting}
...
output elements; (2) \xmlstructref{eicommand}* is a list of commands
to be executed; and (3) \xmlstructref{eiaction}* is a list of actions
to be declared.
%
