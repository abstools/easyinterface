...
% 
  The second form is an alternative in which the file \lst{src}
...
%
  ...
  \item \lst{id} is a unique identifier used to ref to this
    application when communicating with the server.

  \item \lst{visible} indicates if this application should be listed
    when the list of available applications is requested --- by
    default it is \xmlstructvalue{true}. Note that even if an
    application is not \emph{publicly} visible, it can be used like
    any other application by thos who know it \lst{id} (you just need
