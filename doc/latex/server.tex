...
%
  ...
  defined by collection of \xmlstructref{exelements} (files,
  directories, and links to github repositories).
%
  The attribute \xmlstructattr{id} is a unique identifier that is use
  to refer to this set when communicating with the server.
%
  It is also possible to use the second form, where
  \xmlstructref{cfgfilename} is a file that contains an XML structure
  of the first form. However, in such case the \xmlstructattr{id} is
  take from the second form and can be ignored in the the content of
  \xmlstructref{cfgfilename}.
%
}
{}%r%
